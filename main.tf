...
module "debian_vm" {
  ...
  cpu_cores           = 1
  memory              = 2048
  network_bridge      = proxmox_virtual_environment_network_linux_bridge.external_bridge.name
  storage_size_gb     = 10
  os_image_id         = proxmox_virtual_environment_download_file.release_20230531_debian_12_bookworm_qcow2_img.id
  vm_default_username = "debian"
}
